...
export default function CoverLetterBuilder() {
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Only proceed with cover letter generation if credits were successfully deducted
      if (newCredits >= 0) {
        // Your existing cover letter generation code here
      } else {
      ...
      if (error.message === 'Insufficient credits') {
        toast.error(
          <div className="flex flex-col">
            <span className="font-semibold">Cannot Generate Cover Letter</span>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-semibold text-white">Create Cover Letter</h2>
        <div className="text-sm text-gray-400">
          Credits: <span className={`font-medium ${!hasEnoughCredits ? 'text-red-400' : 'text-white'}`}>
            {userData?.credits || 0}
          </span>
          <span className="text-xs ml-2">(10 credits per cover letter)</span>
        </div>
      </div>
